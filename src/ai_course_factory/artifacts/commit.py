...
class ArtifactStorageError(ArtifactCommitError):
    """A storage or persisted-data failure at the Artifact boundary."""
...
class ArtifactRepository(Protocol):
    """Structural contract shared by in-memory and durable repositories."""

    def commit(self, candidate: ArtifactCandidate) -> ArtifactReference:
        """Commit one validated Candidate and return its exact Reference."""

    def get(self, reference: ArtifactReference) -> ArtifactVersion:
        """Retrieve one immutable Version by its exact Reference."""
...
def _is_freezable_value(value: Any, seen: set[int] | None = None) -> bool:
    ...
    if isinstance(value, ArtifactReference):
        return _is_valid_reference(value)
    if seen is None:
        seen = set()
    marker = id(value)
    if marker in seen:
    ...
    if isinstance(value, Mapping):
        seen.add(marker)
        result = all(
            _is_freezable_value(key, seen)
            and _is_freezable_value(item, seen)
    ...
    if isinstance(value, (list, tuple, set, frozenset)):
        seen.add(marker)
        result = all(_is_freezable_value(item, seen) for item in value)
        seen.remove(marker)
        return result
    return False


def _freeze_checked(value: Any, label: str) -> Any:
    try:
        return freeze_value(value)
    except Exception:
        raise CandidateValidationError(f"{label} contains an unsupported mutable value") from None


def _validate_candidate(candidate: ArtifactCandidate) -> None:
    try:
        ...
        if not _is_freezable_value(candidate.payload):
            raise CandidateValidationError("payload contains an unsupported mutable value")
        _freeze_checked(candidate.payload, "payload")
        ...
        if not all(_is_freezable_value(item) for item in candidate.provenance):
            raise CandidateValidationError("provenance contains an unsupported mutable value")
        for item in candidate.provenance:
            _freeze_checked(item, "provenance")
        for dependency in candidate.dependencies:
            if not _is_valid_reference(dependency):
                raise CandidateValidationError("dependencies must be exact Artifact References")
        if candidate.prior_reference is not None and not _is_valid_reference(candidate.prior_reference):
            raise CandidateValidationError("revision predecessor must be an exact Artifact Reference")
    except CandidateValidationError:
        raise
    except Exception:
        raise CandidateValidationError("candidate contains an unsupported mutable value") from None
...
def _validate_revision(
    ...
    if prior.version != current_version or not predecessor_exists:
...
class ArtifactCommitBoundary:
    ...
    def _validate_revision(
        ...
        _validate_revision(candidate, key, current_version, candidate.prior_reference in self._versions)

    @staticmethod
    def _is_valid_reference(reference: Any) -> bool:
        return _is_valid_reference(reference)

    @classmethod
    def _is_freezable_value(cls, value: Any, seen: set[int] | None = None) -> bool:
        """Allow only recursively copyable shapes that ``freeze_value`` seals."""
        return _is_freezable_value(value, seen)
